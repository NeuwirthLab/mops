...
def traverse_directory(root_dir, read_darshan_parsed_file):
    ...
    cal_bandwidth(df)
    add_flops(df)
    cal_fpio_intensity(df)
    df = add_column_to_df(df, 'FLOPSS', [60230.7472*10**6, 49845.6258*10**6, 11361.7806*10**6, 2071.2766*10**6])
    return df


def add_column_to_df(df, column_name, column_values=[]):
...
def add_flops(df):
    # Add FLOPS column to the DataFrame
    df['FLOPS'] = df['Filename'].apply(lambda x: 2*(float(x.split('x')[0])**2) + 2* float(x.split('x')[0]))
    # df['FLOPS'] = df['Filename'].apply(lambda x: x.split('x')[0])
    return df


def cal_fpio_intensity(df):
    df['FPIO_INTENSITY'] = df['FLOPS'] / (df['POSIX_BYTES_READ'] + df['POSIX_BYTES_WRITTEN'])
    df['FPIO_INTENSITY_READ'] = df['FLOPS'] / df['POSIX_BYTES_READ']
    df['FPIO_INTENSITY_WRITE'] = df['FLOPS'] / df['POSIX_BYTES_WRITTEN']
    return df
...
def append_manuel_peak(df):
    ...
    # set to cpu peak instead of mean frame_per_sec
    frame_per_sec_mean = 270174.87 * 10**6
    dict = {
        'FLOPS': [270174.87 * 10**6, frame_per_sec_mean, 270174.87 * 10**6],
        'POSIX_BANDWIDTH': [102.4* 10**9, 1250 * (1000 ** 2), 500 * (1000 ** 2)],
        'FLOPS_READ': [270174.87 * 10**6, frame_per_sec_mean, 270174.87* 10**6],
        'POSIX_BANDWIDTH_READ': [102.4* 10**9, 1250 * (1000 ** 2), 1100 * (1000 ** 2)],
        'FLOPS_WRITE': [270174.87 * 10**6, frame_per_sec_mean, 270174.87* 10**6],
        'POSIX_BANDWIDTH_WRITE': [102.4* 10**9, 1250 * (1000 ** 2), 720 * (1000 ** 2)],
        'Filename': ['RAM', '10G-Ethernet', 'IOR']
    }
    # dict = {
...
def draw_roofline(df=None, start=10**-3, end=10**4, is_read=False, is_aggregated=False, df_peaks=None, only_app=False):
    ...
    if not only_app:
        if df_peaks is None:
            ...
            for index, row in df_peaks.iterrows():
                ...
                la = ""
                if is_aggregated:
                    y = np.minimum(x * row['POSIX_BANDWIDTH'], row['FLOPS'])
                    plt.plot(x, y, label=f"{row['Filename']}")
                    la = f"%s MB/s" % format(row['POSIX_BANDWIDTH'] / 1000 / 1000, '.0f')
                else:
                    if is_read:
                        y = np.minimum(x * row['POSIX_BANDWIDTH_READ'], row['FLOPS_READ'])
                        plt.plot(x, y, label=f"{row['Filename']}")
                        la = f"%s MB/s" % format(row['POSIX_BANDWIDTH_READ'] / 1000 / 1000, '.0f')
                    if not is_read:
                        y = np.minimum(x * row['POSIX_BANDWIDTH_WRITE'], row['FLOPS_WRITE'])
                        plt.plot(x, y, label=f"{row['Filename']}")
    ...
    if is_aggregated:
        x_values = df['FPIO_INTENSITY']
        y_values = df['FLOPSS']
        bw_values = df['POSIX_BANDWIDTH']

    else:
        if is_read:
            x_values = df['FPIO_INTENSITY_READ']
            y_values = df['FLOPSS']
            bw_values = df['POSIX_BANDWIDTH_READ']
        if not is_read:
            x_values = df['FPIO_INTENSITY_WRITE']
            y_values = df['FLOPSS']
    ...
    for i, (x, y, filename, bw) in enumerate(zip(x_values, y_values, filenames, bw_values)):
        app_list.append(
            plt.scatter(x, y, color=colors[i], label=f"{filename} [{bw / 1000 / 1000:.2f} MB/s {y:.2f} FLOPS]"))
    ...
    plt.ylabel("Operational Performance [FLOP/S]")
...
if __name__ == '__main__':
    root_dir = '/Users/zhaobinzhu/research-stuff/repos/mops/darshan-logs/new_consumer'
